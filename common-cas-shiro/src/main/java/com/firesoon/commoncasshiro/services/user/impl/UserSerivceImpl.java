package com.firesoon.commoncasshiro.services.user.impl;

import com.alibaba.druid.support.json.JSONUtils;
import com.firesoon.commoncasshiro.services.user.UserService;
import com.firesoon.dto.user.User;
import com.firesoon.commoncasshiro.mapper.user.UserMapper;
...
public class UserSerivceImpl implements UserService {
    ...
    public User findUserByLogin(String loginname) {
        User user = mapper.findUserByLogin(loginname);
        try {
            String sql = "select count(1) \"organnum\" from model.PATIENT_ACCOUNTING where user_organ is not null";
            Map<String, Object> map = jdbcTemplate.queryForMap(sql);
            if (Integer.parseInt(map.get("organnum").toString()) == 0) {
                user.setUser_organ(null);
            }
        }catch (Exception e){
            //报错可以不管
            e.printStackTrace();
